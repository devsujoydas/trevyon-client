...
const Product = ({ product }) => {
  ...
  return (
    ...
    >
      {/* Image Wrapper */}
      <div className="relative w-full  rounded-lg overflow-hidden border border-zinc-200">
        {/* Product Image */}
        <motion.img
          src={product?.image}
          alt="Product"
      ...
      <div className="mt-4 space-y-0.5 md:space-y-1.5">
        <p className="text-gray-600 md:text-[16px] text-sm c">{product?.productInfo?.category}</p>
        <Link to={`/products/${product?._id}`} className=" font-semibold text-gray-900 md:text-xl text-[18px] c group-hover:text-[#F092B0] transition-all">{product?.name.slice(0, 20)}</Link>
        <p className="md:text-3xl text-lg font-bold text-gray-900 ">$65</p>

        <div className="flex items-center gap-2  text-sm md:text-lg">
          {product?.rating?.average} <IoStar className="inline-block" />{" "}
          <span className="text-gray-500 text-sm">({product?.rating?.reviews.length})</span>
        </div>
      </div>
    </motion.div>
  );
